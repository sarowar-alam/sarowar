...
// externalBaseURL for generating short URLs with external address
var externalBaseURL = getEnv("EXTERNAL_BASE_URL", "http://localhost:8000")
...
func initDB() {
	var err error
	// Use /app/data for database file to avoid conflict with binary
	db, err = sql.Open("sqlite3", "/app/data/go.db")
	if err != nil {
		log.Fatal(err)
	}
...
func createShortURL(c *gin.Context) {
	...
	response := ShortenResponse{
		ShortCode: shortCode,
		ShortURL:  externalBaseURL + "/" + shortCode, // Use external base URL
		LongURL:   req.LongURL,
	}
...
func main() {
	...
	log.Printf("Go service starting on :8000")
	log.Printf("External base URL: %s", externalBaseURL)
	r.Run(":8000")
}
