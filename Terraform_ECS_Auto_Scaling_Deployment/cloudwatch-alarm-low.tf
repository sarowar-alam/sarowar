resource "aws_cloudwatch_metric_alarm" "sqs_alarm_low" {
  ...
  alarm_actions       = [aws_appautoscaling_policy.ecs_service_scaling_IN_policy.arn]
  alarm_description   = "sqs-queue-depth-low-${var.ecs_service_name}"
  dimensions = {
...
resource "aws_appautoscaling_policy" "ecs_service_scaling_IN_policy" {
  name               = "ScaleInPolicy"
