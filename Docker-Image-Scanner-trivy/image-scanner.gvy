                ...
                script {
                    ...
                    try {
                        checkout([
                            ...
                            userRemoteConfigs: [[
                                url: "${params.git_url}",
                                credentialsId: 'jenkins-git-creds'  // Ensure this credential ID exists in Jenkins
                            ]]
                        ])
            ...
            steps {
                script {
                    ...
                    if (params.build_method == 'dockerfile_in_path') {
                        dockerBuildCommand = "cd ../ && docker build -f ${params.dockerfile_path}/Dockerfile -t ${params.image_name}:${env.BUILD_ID} ${env.build_context_path}"
                        echo "Using Dockerfile path: ${params.dockerfile_path}/Dockerfile"
                    } else {
                        dockerBuildCommand = "docker build -f Dockerfile -t ${params.image_name}:${env.BUILD_ID} ${env.build_context_path}"
